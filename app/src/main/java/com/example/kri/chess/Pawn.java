...
public class Pawn{
    ...
    private List<float[]> pawnMesh;
    private VertexArray pawnData;
    public Pawn(Context context){
        pawnMesh = new ArrayList<>();
        pawnMesh = ObjMtlParser.getMesh(context, "pawn.obj");
        pawnData = new VertexArray(pawnMesh.get(0));
    }
    ...
    public void draw(ColorShaderProgram colorShaderProgram, int color){
        ...
        pawnData.setVertexAttribPointer(0, colorShaderProgram.getPositionAttributeLocation(), 3, 24);
        pawnData.setVertexAttribPointer(12, colorShaderProgram.getNormalAttributeLocation(), 3, 24);
        glDrawArrays(GL_TRIANGLES, 0, pawnMesh.get(0).length/6);
    }
}
