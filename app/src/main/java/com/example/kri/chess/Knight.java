...
public class Knight{
    private List<float[]> knightMesh;
    private VertexArray knightData;
    public Knight(Context context){
        knightMesh = new ArrayList<>();
        knightMesh = ObjMtlParser.getMesh(context, "knight.obj");
        knightData = new VertexArray(knightMesh.get(0));
    }
    ...
    public void draw(ColorShaderProgram colorShaderProgram, int color){
        ...
        knightData.setVertexAttribPointer(0, colorShaderProgram.getPositionAttributeLocation(), 3, 24);
        knightData.setVertexAttribPointer(12, colorShaderProgram.getNormalAttributeLocation(), 3, 24);
        glDrawArrays(GL_TRIANGLES, 0, knightMesh.get(0).length/6);
    }
}
